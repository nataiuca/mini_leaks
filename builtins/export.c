...
static void save_argument(char *arg[2], char *argument, char **q, int s)
{
    // Se coloca el terminador en la posición s (separa la cadena)
    argument[s] = '\0';
    // arg[0] contendrá la parte anterior al '='
    arg[0] = *q;
    // Avanza el puntero *q para que apunte a la parte posterior al '='
    *q = *q + s + 1;
    // Si el segmento restante no es una cadena vacía, lo asignamos a arg[1]
    if (**q != '\0')
        arg[1] = *q;
}
